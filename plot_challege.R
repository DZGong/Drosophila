...
install.packages("RMKdiscrete")
library(RMKdiscrete)
empirically_determined_lambda2 <- 0.53214
lambda1_sowbug <- lambda["sowbug"] (1 - empirically_determined_lambda2)
sowbug_LGP <-dLGP(x=0:17,theta=lambda1_sowbug,lambda=empirically_determined_lambda2)
...
library(RMKdiscrete)
weevil_lamb2 <- 0.1
weevil_lamb1 <- lambda_weevil*(1 - weevil_lamb2)
weevil_LGP <- dLGP(x=0:4, theta = lambda_weevil, lambda=0)
...
p_weevil_egg2 <- p_weevil_egg2 + geom_line(aes(y=weevil_LGP),color = "green") +
  geom_point(aes(y = C_count_of_beans_with_k_eggs/sum_weevil), color = "blue")
p_weevil_egg2